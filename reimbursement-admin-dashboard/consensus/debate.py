...
HOW IT WORKS  (live by default; deterministic arbitration)
    The debate runs in three rounds. By default (live=True) each agent's
    opening position comes from a REAL Orchestrator job against the actual
    deployed processes in `Shared/ReimbursementIndependentSolution` --
    ReimbursementClassificationAgent, FraudIntegrityAgent, PolicyRuleCheckWorkflow
    -- via `live_agents.py`. The final consensus is likewise the real, deployed
    `ConsensusArbitrationWorkflow`, not a local re-implementation. Set
    CONSENSUS_ENGINE_OFFLINE=1 (or run_debate(..., live=False)) to fall back to
    the local Python ports in `agents.py` instead (e.g. no network / no
    `uip login`).
...
from .live_agents import classify_live, fraud_screen_live, policy_check_live, arbitrate_live
...
def run_debate(claim: dict, history: list[dict] | None = None, live: bool | None = None) -> dict:
    ...
    live=True (the default) sources cls/frd/pol from the ACTUAL deployed
    Orchestrator processes (ReimbursementIndependentSolution) via real jobs;
    live=False uses the local Python ports in agents.py instead.
    """
    history = history or []
    ...
    if live:
        ...
        frd = fraud_screen_live(claim, history)
    ...
    # ==================== ROUND 3 - CONSENSUS ==================== #
    if live:
        recommendation, path, rationale = arbitrate_live(cls, frd, pol)
    else:
